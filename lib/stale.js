...
module.exports = class Stale {
  ...
  parseTime(ourformat) {
    var modifiedformat = ourformat;
    print("modifiedformat 0 " + modifiedformat)
    var timeformats = ["d", "h", "m", "s"];
    while (modifiedformat != "") {
      for (var formatcount = 0; formatcount < timeformats.length; formatcount++) {
        if (modifiedformat.indexOf(timeformats[formatcount]) != -1) {
          console.log("modifiedformat: " + modifiedformat.substring(modifiedformat.indexOf(timeformats[formatcount])-2, modifiedformat.indexOf(timeformats)))
  ...
  getStaleTime(labelName, createdTime) {
    console.log("doing it")
    var labelFile = fs.readFileSync('labels.json')
    var labelJSON = JSON.parse(labelFile)
    for (var index = 0; index < Object.keys(labelJSON).length; index++) {
      if (Object.keys(labelJSON)[index] == labelName) {
        return (this.parseTime(labelJSON[Object.keys(labelJSON)[index]]["timeout"]) + createdTime)
      }
    }
  }
