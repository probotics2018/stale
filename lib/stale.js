...
module.exports = class Stale {
  constructor(github, {
    owner, repo, logger = console, ...config
  }) {
    this.github = github;
    this.logger = logger;
  ...
  parseTime(type, ourformat) {
    let modifiedformat = ourformat;
    this.logger.info('modifiedformat 0 ' + modifiedformat);
    ...
    while (modifiedformat !== '') {
      for (let formatcount = 0; formatcount < timeformats.length; formatcount++) {
        if (modifiedformat.indexOf(timeformats[formatcount]) !== -1) {
          this.logger.info('modifiedformat: ' + modifiedformat.substring(modifiedformat.indexOf(timeformats[formatcount]) - 2, modifiedformat.indexOf(timeformats)));
          console.log(modifiedformat.substring(modifiedformat.indexOf(timeformats[formatcount]) - 2, modifiedformat.indexOf(timeformats)))
          switch (modifiedformat.substring(modifiedformat.indexOf(timeformats[formatcount]) - 2, modifiedformat.indexOf(timeformats))) {
            case 'd':
          ...
          modifiedformat = modifiedformat.substring(modifiedformat.indexOf(timeformats) + 1, modifiedformat.length);
  ...
  getStaleTime(type, labelName, createdTime) {
    console.log("doing it")
    var labelFile = fs.readFileSync('labels.json')
    var labelJSON = JSON.parse(labelFile)
    for (let index = 0; index < Object.keys(labelJSON).length; index += 1) {
      if (Object.keys(labelJSON)[index] == labelName) {
        var ret = (this.parseTime(type, labelJSON[Object.keys(labelJSON)[index]].timeout) + createdTime);
        return ret;
      }
    }
  }
