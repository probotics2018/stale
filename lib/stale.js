...
module.exports = class Stale {
  ...
  getStaleTime(type, labelName, createdTime) {
    const labelFile = fs.readFileSync('labels.json')
    const labelJSON = JSON.parse(labelFile)
    for (let index = 0; index < Object.keys(labelJSON).length; index += 1) {
      if (Object.keys(labelJSON)[index] == labelName) {
        const ret = parseInt(this.parseTime(type, labelJSON[Object.keys(labelJSON)[index]].timeout)) + parseInt(createdTime.toString().substring(0, (createdTime.toString().length - 3)));
        return ret;
      }
    }
  }
  ...
  search(type, days, query) {
    ...
    // We should modify the query so that it only returns things
