...
module.exports = class Stale {
  ...
  getStale (type) {
    ...
    const days = this.getConfigValue2(type, 'days') || this.getConfigValue2(type, 'daysUntilStale')
    return this.search(type, days, query)
  }

  getClosable (type) { //change to watch labels
 		//get label
 			//returns the time(in parsed time)
 			//time created for issues/prs

 			//return labelName, createdTime
    
     /*const label = this.issues.getLabel(owner,repo,labelName)
     if(label)
     	return console.log("LABELLLLLS"+ label)
...
*/
    ...
    const days = this.getConfigValue2(type, 'days') || this.getConfigValue2(type, 'daysUntilClose')
    return this.search(type, days, query)
  }
  ...
  getConfigValue2(type,key){

    const {owner, repo} = this.config
    val=100000;
//check for exempt first 
/*for(j=0;j<this.config["exemptLabels"].length;j++)
{
	if(this.github.issues.getLabel({owner, repo, name:this.config["exemptLabels"][j]}))
{
	return;
}
else*/ 

//check for label in the labels array 
    for(var i=0;i<this.config["Labels"].length;i++)
    {
      console.log('it is not!!!!!!!!!')
      console.log('is working????????')
      return console.log(this.config["Labels"][i])
      }
      /*
    	///need to get the name at this code, not right yet
    	if (this.github.issues.getLabel({owner, repo, name:this.config["Labels"][i]}))
    	{ 
  			if (this.config["Labels"][i] && typeof this.config["Labels"][label][key] !== 'undefined'){
  				val = min(val,this.config["Labels"][label][key])
  			}
  		}
  	}
    if (val<100000)
  ...
  async ensureStaleLabelExists (type) {
    const {owner, repo} = this.Configured
    const staleLabel = this.getConfigValue(type, 'staleLabel')
